...
class DatabaseHelper {
  ...
  Future<void> insertCombustivel(Map<String, dynamic> combustivel) async {
    final db = await database;
    await db.insert('combustiveis', combustivel);
  }

  Future<List<Map<String, dynamic>>> getCombustiveis() async {
    final db = await database;
    return await db.query('combustiveis');
  }

  Future<void> deleteCombustivel(int id) async {
    final db = await database;
    await db.delete(
      'combustiveis',
      where: 'id = ?',
...
Future<void> deleteDestinoByName(String nome) async {
  final db = await database;
  await db.delete('destinos', where: 'nome = ?', whereArgs: [nome]);
}
